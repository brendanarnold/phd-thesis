...
To understand when the \ac{SDW} state occurs we first consider the response of a gas of non-interacting Fermions subject to a perturbing magnetic field. The resulting equation is known as the Lindhard susceptibilty and is often quoted as\footnote{See pg 81 ff. of Dressel \cite{Dressel2002} for a full derivation.},
\begin{equation}
...
\end{equation}
and is the matrix transition element for the scattering process. The numerator term contains two Fermi functions --- the same as eqn.~\ref{Eqn:Theo:FermiFunction} --- which ensure that the susceptibilty is finite for states which scatter across the Fermi energy and zero if they do not - consquently, the Lindhard susceptibility models electron-hole scattering in particular. They also smear the susceptbility dispersion as a function of temperature. The third term in the denominator corresponds to the excitation energy of the perturbing field with $\omega$ corresponding to the temporal frequency of the field. The final term in the denominator is an artefact of the adiabatic approximation used to calculate the perturbation with the completed approximation taking the limit of $\delta \to 0$. The first sum in the Lindhard function is over all $k$ states in the first \ac{BZ}, the second sum combines each energy band. The real and imaginary parts of equation \ref{Eqn:Intro:Lindhard} are,
\begin{align}
