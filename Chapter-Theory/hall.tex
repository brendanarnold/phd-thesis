...
\begin{equation}
    \sigma_{ij} = \frac{e^2}{4 \pi^3 \hbar^2}\int \partial k_\perp \int^{2\pi}_{0} \partial \phi \int^{\infty}_{0} \partial \phi^{\prime} v_i(\phi) v_j(\phi - \phi^{\prime})\frac{m^*}{\omega_c}e^{-\phi^{\prime}/(\omega_c \tau)}
\end{equation}
where $k_\perp$ and $\phi$ are the cylindrical co-ordinates along the field axis and $\phi^{\prime}$ is an angular integration variable representing the quasiaprticle path. $v_i$ and $v_j$ are the Fermi velocities in the $i$ and $j$ direction respectively. From this integral it is possible to determine the conductivity tensor for a variety of Fermi surface geometries, however given the shape of the \ac{BSCO} Fermi surface, we are most interested in the cylindrical Fermi surface which gives the following conductivity tensor, $\rho$ for a magnetic field applied along $z$,
\begin{equation}
