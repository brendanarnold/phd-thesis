...
A more detailed analysis of this process follows, beginning with the mathematical foundation for oscillations in the magnetisation.

\subsubsection{Oscillations in magnetisation}

We begin by calculating the degeneracy of the Landau tubes i.e. the number of electron states per tube. Because the states under a magnetic field are a one-to-one rearrangement of the states with no field, we can use the Sommerfeld number of states per unit k-space ($V/4\pi^3$) to determine the degeneracy. From the Onsager relation (eqn.\ref{Eqn:2:Onsager}) we see that the additional area for successive tubes is $\Delta a_{k_{\perp}}  = 2\pi e H/\hbar$ which we can convert to a volume for each tube by integrating over $k_{\perp}$. This gives a degeneracy per tube therefore of,

\begin{equation}
D_{\textrm{tube}} = d k_{\perp}\left(\frac{2\pi e H}{\hbar}\right)\left(\frac{V}{4 \pi^3}\right) = \frac{eHVdk_{\perp}}{\hbar 2\pi^2}
\end{equation}

An interesting aside is that the degeneracy of each of the tubes increases proportional to $H$, wheras the number of levels $r$ below the Fermi surface is proprtional to $1/H$ and so the number of states below the Fermi surface remains \textit{approximately} the same with small fluctuations due to the discrete nature of $r$. This changes when the last Landau tube passes the Fermi surface at which point we have magnetic breakdown.

We continue with the Landau thermodynamic potential, $\Omega$.
