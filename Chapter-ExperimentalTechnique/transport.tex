...
The most simple and well known method for determining the doping of a material ustilises the so-called `universal' Tallon relation~\cite{Presland1991} which links $T_c/T_c(\textrm{max})$ to $p$ as follows,
\begin{equation}
\label{Eqn:ExpH:TallonRelation}
T_c/T_c(\texrm{max}) = 1 - 8.26 (p - 0.16)^2
\end{equation}
This relation was established base don measurements of \ac{LSCO} and found to agree for \ac{Y123}
Previous determinations of doping have used the technique published by Ando \etal~\cite{Ando2000} in 2000 where samples of \ac{BSCO} were compared with measurements of other cuprates where the carrier concentration is more easily determined. In particular \acf{LSCO} provides a direct relation between the Sr content and the doping. However there is some doubt as to whether it is appropriate to compare \ac{LSCO} and \ac{BSCO} measurements across the superconducting dome given that the van-Hove singularity arising from the crossover between hole-like and electron-like Fermi surfaces occurs at different dopings with \ac{LSCO} occuring at $p=0.18$ and \ac{BSCO} at $p \geq 0.2$.
