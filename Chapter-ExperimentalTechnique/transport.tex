...
\end{equation}
This relation was established based on measurements of \ac{LSCO} and found to agree for \ac{Y123}\TODO{ref}.
...
The final technique is by comparing instead \ac{BSCO} and \ac{Tl2201} which both have similar structures \TODO{and similar position of van Hove?}. Doping in \ac{Tl2201} has been well characterised in the overdoped side through recent quantum oscillation experiments~\cite{Bangura2010} which maps well to where the majority of our \ac{BSCO} samples lie. Here the the doping is determined using the Tallon relation for underdoped to slightly overdoped samples with $T_c/T_c(\textrm{max})$ down to $T_/T_c(\textrm{max})$ $0.71$, below this value a linear relation is used: $T_c/T_c(\textrm{max}) = 2.390 - 7.696p$. Again some a priori knowledge of the approximate location (i.e. overdoped or underdoped) on the superconducting dome is required.
