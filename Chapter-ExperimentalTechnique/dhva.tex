...
\begin{figure}[h!]
    \begin{center}
        ...
        \caption{Left panel shows the angle dependance of the raw torque signal  clearly showing a negative $B^2$ component for $\theta>0$ and a positive $B^2$ component for $\theta<0$. Centre panels show the \ac{FFT} of simulated data containing $\sim\unit[0.5]{mV}$ of noise fitted to a 2nd order polynomial with field on top and inverse field on the bottom. The right panel shows the resulting \ac{FFT}s.}
